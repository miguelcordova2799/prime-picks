  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        {/* Subscription banner */}
        {!isSubscribed && (
          <div className="mb-6 p-5 rounded-xl bg-gradient-to-r from-[#EF9F27]/15 to-[#EF9F27]/5 border border-[#EF9F27]/30 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
            ...
            <Link
              to="/#pricing"
              className="shrink-0 px-5 py-2.5 bg-[#EF9F27] text-black text-sm font-bold rounded-lg hover:bg-[#D4891A] transition-colors flex items-center gap-1"
            >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
          <StatCard icon={Trophy} label="Ganados" value={stats.wins} color="text-emerald-400" />
          <StatCard icon={Target} label="Perdidos" value={stats.losses} color="text-red-400" />
          <StatCard icon={TrendingUp} label="ROI" value={`${Number(stats.roi || 0).toFixed(1)}%`} color={stats.roi >= 0 ? 'text-emerald-400' : 'text-red-400'} />
          <StatCard icon={TrendingUp} label="Total picks" value={stats.total} color="text-[#EF9F27]" />
        </div>
    ...
    <div className="bg-[#111111] border border-white/8 rounded-xl overflow-hidden">
      ...
      <div className="px-4 pb-4 grid grid-cols-3 gap-3 border-t border-white/5 pt-3">
        ...
        <div>
          ...
          <div className="text-sm font-semibold text-[#EF9F27]">{pick.odds}</div>
        </div>
        <div>
          <div className="text-xs text-white/35 mb-1">Edge</div>
          <div className="text-sm font-semibold text-emerald-400">+{pick.edge}%</div>
        </div>
      </div>
